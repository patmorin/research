...
\begin{abstract}
  ...
  In particular, there exists point sets, $V$, such that, for any graph $G=(V,E)$
...
It is known that, for any $d\in O(1)$ and any $\eps >0$, where
$\eps$ may even depend on $n$, it is possible to construct a
graph with $O(n/\eps^{d-1})$ edges such that $\strf(G)\le 1+\eps$
\cite{carmi.smid:optimal,ruppert.seidel:approximating}.\footnote{We
note that not all of the classic spanner constructions have an optimal
dependence on $\eps$ and $d$.  Spanners based on the well-separated pair
decomposition \cite{callahan.kosaraju:faster} can have $\Omega(n/\eps^d)$
edges.}  This dependence on $\eps$ and $d$ is optimal: Elkin and
Solomon \cite{elkin.solomon:steiner} show that, in the worst case,
any graph $G=(V,E)$ having $\strf(G)\le 1+\eps$ has maximum degree
$\Omega(1/\eps^{d-1})$.  A variant of their argument shows that, for
some point sets, a constant fraction of vertices must have degree
$\Omega(1/\eps^{d-1})$ and therefore, $\Omega(n/\eps^{d-1})$ edges
\cite{solomon}.
...
exists a graph with $\asf(G)=1+\eps$ having $O(n/\eps^{d-1})$
...
the dependence on $\eps$ and/or $d$.
...
bulk of the paper.  In \secref{upper-bound}, we also show that these graphs
...
set $V$ of $n$ points in $\R^d$, there exists a spanning tree, $T=(V,E)$,
such that $\asf(T)\in O(\log n)$.  More recently, this was improved by
...
Aldous and Kendall \cite[Section~5.3]{aldous.kendall:short-length} show that,
...
$V\subset[0,\sqrt{n}]^2$, and any $\eps > 0$, there exists a
...
while the second adds $O(n)$ edges
\cite{callahan.kosaraju:faster,salowe:constructing,vaidya:sparse}.
With the addition of these extra edges we have, for any $u,w\in V$
\[
...
$E_1,\ldots,E_{n'}$ are important and why the choice of the representative
...
\begin{proof}
  ...
  still satisfies $\epsilon \ge D\sqrt{k/n}$---which it does.) What
  remains is to show how can be implemented in $O(n\log^d n)$ time.
  ...
  can, for any $\beta >0$ construct a $(1+\beta)$-spanner of $n'$ points
  in $\R^d$ that has $O((1/\beta)^{d-1}n')$ edges in $O((1/\beta)^{d-1}
  n'\log n')$ time.  Using this algorithm with $\beta=1/k^{1/(d-1)}$,
  the $(1+1/k^{1/(d-1)})$-spanner of the $n'$ points in $N$ can be
  constructed in time
  \[
  ...
  (2)~a randomly chosen point in $B$, or (3)~the point with minimum index
  in $B$.\footnote{For this third type of query, we use a constant-time
  range-minimum data structure \cite{bender.farach-colton:lca} as the
  1-dimensional structure.} Furthermore, using a box-point duality that
  maps square boxes in $\R^d$ onto points in $\R^{d+1}$, a set of square
  boxes in $\R^d$ can be preprocessed, using $O(n\log^d n)$ time and
  space, so that, for any query point $p\in\R^d$, we can, in $O(\log^d
  n)$ time, determine the index of the smallest box that contains $p$.
  ...
  Suppose, furthermore, that $|E'_i\cap V|\ge \epsilon n$.  Then, if we
  repeat the above sampling procedure $\alpha\ln n/\epsilon$ times, then,
  with probability at least $1-n^{-\alpha}$, one of our sample points,
...
$1+o_n(1)$ can be constructed in $O(n\log^d n)$ time. In
this section, we show that the running time can be reduced to $O(n\log n)$
provided that we are willing to sacrifice a little on the $o_n(1)$ term
in the average stretch factor.

\begin{thm}\thmlabel{algorithm}
  For every constant dimension, $d$, and every set, $V$, of $n<\infty$
  points in $\R^d$, there exists a randomized $O(n\log^{d} n )$ time
  algorithm that constructs a graph $G''=(V,E)$ that has $O(n)$ edges and,
  with high probability, $\asf(G'')=1+o_n(1)$.
\end{thm}
...
\begin{proof}
  ...
  $(1+(1/k)^{1/(d-1)})$-spanner of $N$ takes $\Theta(n\log^{d-1} n)$ time;
  ...
  These three bottlnecks in the running-time are resolved as follows:
  \begin{enumerate}
     \item We only construct a $(1+(\log^{d-2}
      n/k)^{1/(d-1)})$-spanner of $N$.  Using the algorithm of Ruppert
      and Seidel, the construction of this spanner takes only $O(n\log
      n)$ time.

     \item The sampling process used to find $E_1,\ldots,E_{n'}$
     has two phases.  In the first phase, a range tree is constructed
     that contains the points of $V$.  In the second phase, $O(n'\log
     n/\epsilon)$ queries are performed on this range tree.

     The first phase takes $O(n\log^{d-1} n)$ time.  To speed up this
     phase, we instead construct a range tree, $T_1$, on a Bernoulli sample
     $V'\subseteq V$ where each point is sampled independently with
     probability $p=1/\log^{d-2} n$.  A standard application of Chernoff's
     Bounds \cite[Appendix~A.1]{alon.spencer:probabilistic} shows that,
     with high probability,
     \begin{enumerate}
       \item $T$ can be constructed in $O(n\log n)$ time;
     ...
     which can be computed in $O(\log^{d-1} n)$ time using this range
     tree is an accurately enough estimate of $|B\cap V|$.  

     The second phase requires sampling $n'\log n/\epsilon$ points and,
     for each sample point, $u$, counting the number of points of $V$
     in some box centered at $u$.  By increasing the value of $\epsilon$
     from $\epsilon=1/k$ to $\epsilon=\log^{d-1} n/k$, this sampling
     and counting can be done in $O(n\log n)$ time.  We note that the
     sample obtained this way is a random sample of $V'$, which is
     itself a Bernoulli sample of $V$. Despite this, the sample will
     still, with high probability, sample an element of the box $E'_i$
     described in the proof of \thmref{algorithm}.

    \item In the third phase, we construct a range tree, $T_2$ for
      (the duals of) some boxes in  $\{E_1,\ldots,E_{n'}\}$.  In $T_2$,
      we include every box, $B\in\{E_1,\ldots,E_{n'}\}$ such that $|B\cap
      V'|/p\ge\epsilon n$.  From the discussion, $T_2$ then contains
      only boxes, $B$ such that $|B\cap V|\ge \epsilon n/2$ and $T_2$
      includes every box, $B$ such that $|B\cap V|\ge 2\epsilon n$.
      Determining which boxes to include in $T_2$ requires $n'$ queries in
      $T_1$, so this takes $O(n'\log^{d-1} n)\subset O(n\log n)$ time.
      Building the tree $T_2$ takes $O(n'\log^{d} n)\subset O(n\log
      n)$ time.

      At this point, we would like to use $T_2$ to compute $i(w)$ for
      every point $w\in V$, but this would take $O(n\log^d n)$ time.
      Instead, take a sample, $V''\subseteq V$ of size $n/\log^{d-1} n$
      and compute $i(w)$ for each $w\in V''$.  This takes only $O(n\log
      n)$ time.
      ...
      E_i\cap V$.  To understand this, it is necessary to revisit the
      proof of \thmref{upper-bound}.  The only place in which the choice
      of $w_i$ plays a role is in the analysis of bad Type~4 pairs.
      In this analysis, the property that $w_i$ is chosen as the point
      that minimizes $i(w_i)$ from all $w_i\in E_i\cap V$ is only used
      to establish \clmref{zuper}.

      In the notation of the proof of \clmref{zuper}, the choice of
      $w_i$ is used to establish that $r_{i'} \le r_{i_{j^*}}$ since,
      otherwise $i(w^*) < i(w_i')$, contradicting this choice of $w_{i'}$.
      Returning to our setting, the preceding argument continues to hold
      provided that $w^*$ is in $V''$.  For a particular point $w^*$,
      $\Pr\{w^*\in V''\} = 1/\log^{d-1}n$, which is not strong enough. 
      However, $w^*$ can be chosen as any point that satisfies 
      conditions~B1--B3.

      Now, the number of points that \ldots
   \end{enumerate}
