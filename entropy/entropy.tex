...
then prove the lower bound for the new model.  Refer to \figref{s}.  Let $S_1= \boundary P
...
We have presented data structure for distribution-sensitive point
location in convex subdivisions.  Our data structures achieve, up to
constant factors, the best possible query time for data structures in
the linear decision tree model of computation.  
...
structures is when the distribution $D$ is uniform over a set $S$ of
$m$ points.  Such distributions are easily obtained by sampling some
