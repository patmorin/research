...
Next we define some regions associated with the nodes of $T_S$.  Note that
every node $v$ in $T_S$ is naturally associated with a simplex $\Delta(v)$
that was obtained from \thmref{point-partition} and that generated $v$.
...
Let $S$ be a set of $m$ points in $\R^2$, let $T_S$
denote the partition tree described above, and let $V_i$ denote
the set of nodes of $T_S$ at distance $i$ from the root.
...
\begin{thm}\thmlabel{prob-partition-tree} 
  ...
  probability measure $D$, let $T_D=T_S$ denote the partition tree given
  by \thmref{point-partition-tree}, and let $V_i$ denote the set of nodes
  of $T_D$ at depth $i$.  There exists a constant $c$, independent of $r$,
  such that with probability at least $1-O(m^{\gamma})$, $T_D$ has the
  following properties, for every $i\in\{0,\ldots,\lfloor\alpha\log_r
  n\rfloor\}$:
  \begin{enumerate}
    \item $\{\Xi(v) : v\in V_i\}$ is a partition of $\R^2$,  
...
a non-terminal leaf $w$ after $\lfloor \alpha\log_r m\rfloor=O(\log
...
\begin{lem}\lemlabel{independent}
  ...
  and  in $k$-general position and $|V'|\ge |V|/(cr)^{i(d/k+1-1/d)}$
...
\begin{proof}
  ...
  independently with probability $p=(c'r)^{-i(d/k+1-1/d)}$. We
  will prove that
  \[
     \Pr\left\{
  ...
  \[
    i^{d^2/2}(cr)^{di}\cdot(cr)^{i(k-k/d)}p^k 
     = i^{d^2/2}(cr)^{i(d+k-k/d)}p^k 
     = o(1) \enspace 
  \]
  provided that $c'> c$.  The above argument shows that the nodes in
  ...
  value at least $\lfloor{p|V|}\rfloor=\Omega(|V|/(cr)^{i(d/k+1-1/d)})$.
...
\begin{lem}\lemlabel{lower-bound}
  ...
  and let $T^*$ be some linear decision tree that solves $\mathcal{P}$.
  Then $\mu_D(T^*) = \Omega(H(\{\Xi(v):v\in L\})-1)$.
\end{lem}
