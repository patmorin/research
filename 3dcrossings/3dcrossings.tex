...
\begin{proof}
  ...
  $\binom{k}{2}$ to $\crs_3(G)$.  Let $R_1,\ldots,R_\ell\ge 1$ be
  the sequence that counts the number of edges passing through each
  $p$-grid point, excluding those $p$-grid points with 0 edges passing
  through them.  Therefore, we have $\sum_{i=1}^\ell{R_i}\ge(p-1)m$
  and $\ell \le np^3$.
  Therefore the total
  contribution of crossings that occur on the $p$-grid to $\crs_3(G)$
  is at least
  \[
      \sum_{i}\binom{R_i}{2} \ge \ell \binom{m(p-1)/\ell}{2}
      \ge np^3\binom{m(p-1)/np^3}{2} \ge cm^2/np
  \]
