...
\begin{proof}
  ...
  size at most $(2\sqrt{d}+2)^d\in O(1)$ such that every node in $U$
...
To make the preceding ideas precise, let $D_i$ be the ball centered at
the center of $\Delta_i$ and having radius $cr_i$.  Points of $V$ that
are in $D_i$ can be problematic for $V_i$; there is no guarantee that such
points have paths with $1+o_n(1)$ stretch factor to the points in $V_i$.
...
points is used as the representative vertex, $w_i$, then all $kn/2$
paths from some $u$ in $V_i$ to some $w\in E_j$ will have to make a
detour through $w_i$.  By repeating this for many different values of
$i$, this is enough to produce an average stretch factor significantly
larger than 1.
...
\begin{proof}
  ...
  of pairs of points, $u\in V_i$, $w\in V_j$, $j\le i$, to consider
  (recall that $\Delta_1,\ldots,\Delta_{n'}$ are ordered so that $r_j \le r_i$):
  \begin{enumerate}
  ...
  such that the radius of $\Delta_{i_{j+1}}$ is at least $4c^2$ times
  ...
  contain many points of $V$ and Property~5 of $k$-partitions to prove
  ...
  \begin{figure}
    ...
    \caption{The number of points in $E$ that are assigned to
      $\Delta_{i_{j^*+1}},\ldots,\Delta_{i_{j^*+t}}$
      is only $O(k)$.}
    \figlabel{type4}
  \end{figure}
...
%  But then Property~4 of $k$-partitions implies that $\mu\in O(1)$.
  ...
  obtain 
  \begin{equation}
     C_1t\epsilon kn \le C_2(kn + (tk)^2/\epsilon) \enspace ,  \eqlabel{contra}
  \end{equation}
  for some constants $C_1$ and $C_2$.  \Eqref{contra} yields a contradiction when 
  \[
      t = D/\epsilon
  \]
  and
  ...
  \]
  for some sufficiently large constant $D$.
  ...
  \begin{enumerate}
    ...
    point in $E_{i'}$ is at distance at most $4r_{i_j}/c$ from $w^*$
    and $E'_{i_{j^*+t+C}}$ is centered at $w^*$ and has radius
    $2r_{i_{j^*+t+C}}/c > 4r_{i_j}/c$.
    ...
    define at least one bad pair, this implies that each $\Delta_{i_j}$ is
    ...
    $E_{i'}\subset E'_{i_{j^*+t+C}}$, as promised.

    Since $E_{i'}$ and $E'_{i_{j^*}}$ are disjoint and $E_{i'}\subset
    E'_{i_{j^*+t+C}}$,
    \[
      |V\cap E'_{i_{j^*+t+C}}\setminus E'_{i_{j^*}}| 
    ...
    But, by definition, 
    \[
      |V\cap E'_{i_{j^*+t+2\mu}}\setminus E'_{i_{j^*}}| 
...
\begin{proof}
  ...
  \begin{enumerate}
     ...
     \begin{enumerate}
       ...
       \item $E_1,\ldots,E_{n'}$ are square boxes where each $E_i$
         is a square box of side-length $L(\Delta_i)/c$ that intersects $D_i$.
     \end{enumerate}
...
\begin{proof}
  ...
  argument (see \figref{lower-bound}.b) shows that
  \begin{align}
     \frac{\|uw\|_G}{\|uw\|}
       & \ge \frac{\sqrt{\alpha n}+\sqrt{(n/2)^2+(n/2-\sqrt{\alpha n})^2}}
                {n/\sqrt{2}} \notag \\
       & = \frac{\sqrt{\alpha n}+\sqrt{n^2/2-\sqrt{\alpha}n^{3/2}+\alpha n/2}}
                {n/\sqrt{2}} \notag \\
       & \ge \frac{\sqrt{\alpha n}+n/\sqrt{2}-\sqrt{\alpha n/2}}
               {n/\sqrt{2}} \eqlabel{blobber} \\
       & \ge 1+\Omega(1/\sqrt{n}) \notag \enspace .
  \end{align}
  (Inequality~\eqref{blobber} is obtained by comparing 
   $\left(\sqrt{n^2/2-\sqrt{\alpha}n^{3/2}+\alpha n/2}\right)^2$ and 
   $\left(n/\sqrt{2}-\sqrt{\alpha n/2}\right)^2$.)
...
These highways were expensive and would not have been built unless they
...
metric space in which all points have unit distance, the average stretch
factor of a graph, $G=(V,E)$, having $m$ edges is at least
\[
...
The authors of this paper are partly funded by NSERC and CFI.  The author
are indebted to Shay Solomon for providing helpful feedback on an earlier
