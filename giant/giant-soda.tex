...
\begin{wrapfigure}{R}{0.5\textwidth}
  \centering{\includegraphics[width=0.48\textwidth]{grid}}
...
%\begin{wrapfigure}{R}{0.5\textwidth}
%  \centering{\includegraphics[width=0.48\textwidth]{rft-problem}}
...
\begin{wrapfigure}{R}{0.5\textwidth}
  \centering{\includegraphics[width=0.48\textwidth]{magnification}}
...
\begin{proof}
  ...
  \begin{figure}
    \centering{\includegraphics{robust-1}}
    \caption{The graph $G'$}
    \figlabel{g-prime}
  \end{figure}
  ...
  \begin{figure}
    \centering{\includegraphics{robust-2}}
...
times on the initial value $k$, i.e., $f^{i}(k) = \underbrace{f(f(f(\cdots
f}_{i}(k)\cdots)))$.  We use the convention that $f^0(x) = k_0$ for
all $x$.  We define the \emph{iterated $f$-inverse function} $f^*(n) =
\max\{i : f^{i}(k_0) \le n\}$.
...
The proof, which is similar to the proof of \thmref{klogk-1d} is in 
\appref{proofs}.
...
\begin{proof} 
  ...
  \begin{figure}
    \centering{
     \begin{tabular}{cc}
       \includegraphics[width=0.45\textwidth]{dumb-1} &
       \includegraphics[width=0.45\textwidth]{dumb-2}
     \end{tabular}
    }
    \vspace{-1ex}
    \caption{Partition a dumbbell tree into
      components of size $k$ using $O(n/k)$ nodes (left).  The set $S$
      (whose elements are denoted by \textbullet) kills $O(|S|k)$
      vertices in each dumbbell tree.}
...
%  \end{figure}
  ...
  Therefore, %a straightforward application of triangle inequality implies
  %that $\|xy\|_{G\setminus S} \le t\|xy\|$.
  \begin{align*}
    \|xy\|_{G\setminus S} 
...
of robustness except that the size of $S^+$ may also depend on $|V|$.  In particular, any $f(k)$-robust $t$-spanner is also an $f'(k,n)$-hardy $t$-spanner with $f'(k,n)=f(k)$.  The proof of the following theorem is given in \appref{proofs}.
...
upper bound, one can study the $\sqrt{n}\times\sqrt{n}$ grid graph;
see \figref{grid}.  An application of the probabilistic method, based on
randomly shifting a quadtree, shows that this graph is an $O(k^2)$-robust
$3$-spanner.  Therefore, the vertices of the $\sqrt{n}\times\sqrt{n}$
grid admit a linear-size $O(k^2)$-robust 3-spanner.  In contrast,
\thmref{general-lower-bound-1d} shows that any $f(k)$-robust $t$-spanner
for the $1\times n$ grid has superlinear size.  This suggests that one
dimension is the hardest case:
...
\begin{proof}[Proof of \thmref{general-1d}]
  The proof is very similar to the proof of \thmref{klogk-1d}, but
  makes use of some properties of the iterated $f$-inverse function
  from \secref{iterated}.  We make use of the following two properties of
  iteration on $f$.  For any $k> k_0$, there exists $i$ such that
  \[
  ...
  \]
  The second propert we use is that, since $f(k)$ is increasing
  and convex, $f(x)/x$ is non-decreasing: For every $\delta\ge 0$,
  $f(x+\delta)/(x+\delta) \ge f(x)/x$.

  Let $S$ be any non-empty subset of $V$ and let $k=|S|$.   Select a
  random integer $r\in\{0,\ldots,\upen{f^{f^*(n)+1}(k_0)}-1\}$.
  We consider the subgraph $G'$ of $G_f$ that contains only the edges
  $x_ix_{i+\ell}$ where $i-r$ is an integer multiple of $\ell$.  We say
  that an edge $x_ix_{i+\ell}$ has \emph{span} $\ell$.
  ...
  \end{eqnarray*}
  \begin{figure}
    \centering{\includegraphics{spanjump}}
...
\begin{proof}[Proof of \thmref{linear-size}]
