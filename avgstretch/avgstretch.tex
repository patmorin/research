...
factor can be constructed efficiently.  In this section, we present an
$O(n\log n)$ time Monte-Carlo algorithm that, with high probability,
constructs a graph with average stretch factor $1+o_n(1)$.  The
...
%  But then Property~4 of $k$-partitions implies that $\mu\in O(1)$.
  ...
  \]
  (The $O(kn/\binom{n}{2})$ term and the $\epsilon n^2/\binom{n}{2}$ term are dominated by the three other terms, which are all in $O((\log n/n)^{1/(2d+1)})$.)
\end{proof}
...
\begin{proof}
  ...
  $D_i$, and $E_i$ shows that the graph $G'$ has $\asf(G')$ satisfies the
...
\begin{thm}\thmlabel{algorithm-faster}
  ...
  \[
      \asf(G'')=\begin{cases}
         1+ O((\log^{7} n/n)^{1/5}) & \text{for $d=2$} \\
         1+ O((\log^{(d-2)/(d-1)/n^{1/(4d-4)}}) 
                  & \text{for $d\ge 3$} 
      \end{cases}
  \]
\end{thm}
