...
of \thmref{containment} combined with a careful random sampling strategy
provides a solution. 
...
Computing $C_S(s)$, using \lemref{cover-algorithm} takes $\Oe((m_sn)^{2/3})$
...
\lemref{count} to bound the quality of the approximation.  This results
in a more efficient space/accuracy tradeoff than that provided by
\lemref{absoluste-a}.  The cost of this savings in space is that we
...
\thmref{triangle-inclusion}, with $k=(cm/n)^{1+\alpha}$, then we obtain a
...
construct $C(S)$ in $O(m)$ time using Part~1 of \lemref{construction}
