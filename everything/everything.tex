...
\Secref{analysis} proves that this data structure matches the query
...
For a node $v$ of $T$ whose ancestors are $v_0, v_1,\ldots,v_i$
we define $\poly(v)=\Delta(v)\cap\bigcap_{j=0}^i \Delta(v_i)$.  Note that
...
\begin{thm}\thmlabel{point-partition-tree}
  ...
  \begin{enumerate}
    \item $\{\Xi(v) : v\in V_i\}$ is a partition of $\R^d$,  
    \item For every node $v\in V_i$, $|S\cap\Xi(v)| \le m(2/r)^i$, and 
    \item For any hyperplane $\ell$, the number of nodes in $v\in V_i$
      such that $\ell$ intersects the interior of $\poly(v)$ is at most
      $(cr^{1-1/d})^i$.
    \end{enumerate}
...
for some parameter $\tau > 0$.

Next, we trim some nodes of $T_{\mathcal{P},D}$.  First, we elminate
all nodes of depth greater than $\delta\log_{r/3} m$.  Next, we
use the Interference Oracle to test, for each node $v$ of $T_D$,
if $\mathcal{P}(p)=\mathcal{P}(q)$ for all pairs of points $p,q\in
...
into $O(((d+1)\depth_T(v))^{d/2})=\log^{O(1)} n$ simplices.

\begin{lem}\lemlabel{filter-tree-construction}
For any $n>0$ and any $\tau > 0$, a filter tree of size $m=n^\tau$ having
...
Using a filter tree to answer a query $q\in \R^d$, is easy: We follow
...
the performance of filter trees.
...
$T_{\mathcal{P},D}$.  We accomplish this by decomposing the nodes
...
\begin{proof}
  ...
  disjoint. The set $V'$ satisfies the conditions of the lemma.
\end{proof}
...
\begin{lem}\lemlabel{lower-bound}
  ...
  Conditions~1--3 of \thmref{prob-partition-tree}.  Let $L$ denote the
  set of leaves of $T_{\mathcal{P},D}$, and let $T^*$ be any linear
...
\begin{proof}
  ...
  For a set $V$ of nodes $T_{\mathcal{P},D}$, we define
  $H(V)=H(\{\Xi(v):v\in V\})$
  ...
  1/2^{i}$.  Note that, because the maximum depth of a node is
  $\gamma\log_{r/3} m$, the depth of a node in $G_i$ is at most
  $i/\log(r/3) + O(1)$.

  We then further partition each group $G_i$ into subgroups
  $G_{i,1},\ldots,G_{i,t_i}$.  Let $\poly_{i,j}=\{\poly(v):v\in
  G_{i,j}\}$.  We partition $G_i$ into subgroups $G_{i,j}$ where, for
  $j\in\{1,\ldots,t_i-1\}$, we have $|G_{i,j}|= \Omega(2^{\alpha i})$,
  for some constant $\alpha >0$ and the elements of $\poly_{i,j}$
  are pairwise disjoint and in $k$-general position.  Furthermore,
  the final subgroup, $G_{i,t_i}$ has size at most $O(2^{\beta i})$,
  ...
  $\poly(w)$ intersects at most $ik$ elements of $\poly_{i,j}$ since,
...
\begin{thm}
  ...
  $O(n^\epsilon)$ calls to the Interference Oracle, a filter tree can
  be constructed in $O(n^\epsilon)$ time and $O(n^\epsilon)$ space.
  This filter tree can, in conjunction with the backup oracle, answer
...
\begin{proof}
  Applying \lemref{filter-tree-construction} with $\tau < \epsilon$
  ...
  \thmref{prob-partition-tree} then, by \lemref{lower-bound},
  ...
  \thmref{prob-partition-tree}) to answer a query using a filter tree is
  \[
     (1-O(n^{-\tau\delta})) O(H(L)) + O(n^{-\tau\delta}) O(\log n) =
        O(H(L)+1)  \enspace .
  \]
  On the other hand, by \lemref{lower-bound} the expected time required
...
\begin{thm}
  There exists a distribution-sensitive data structure for
  ray shooting in a $d$-dimensional polytope that uses
  $O(n^{\floor{d/2}+\epsilon})$ preprocessing time and space.
\end{thm}
