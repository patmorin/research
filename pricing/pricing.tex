...
\usepackage{amsthm,amsfonts,amsmath,graphicx}
\usepackage{pat}
...
Refer to \figref{intro}.  A product $P=(p,q_1,\ldots,q_d)$ is defined
...
%The second type of model, which is also the type of model considered in
%the current paper, involves a variable and large number of parameters,
%each of which is a measure of real-world data point.  One such result
%by Bhattacharya \cite{bXX}, considers optimizing an objective function
%within the context of the media (advertising) marketing mix.  Here, the
%product is fixed and one wants to allocate an advertising budget over
%several media (radio, television, magazine, internet,\ldots) in order
%to maximize profit.  Here each media type defines a demand curve where
%more advertising implies higher demand and the total demand is the sum
%of demands generated by all media types.  He implements an optimization
%algorithm as a SAS macro that can be used to obtain a ``close to'' global
%solution by evaluating the objective function at each point of a regular
%grid.  This algorithm uses $O(r^n)$ evaluations, where $1/r$ represents
%the granularity of the sampling and $n$ is the number of media variables.
...
$\textsc{ProductDesign}(1)$ (\secref{1-d}), and $O(n(\log n)^{d+1})$ time
approximation schemes for $\textsc{ProductDesign}(d)$ (\secref{2-d}
...
$q_i^*$ can be safely discarded from $L$ since, by \lemref{monotone},
$\val(p,q^*_i)$ will never again exceed $\val(p,q^*_{i+1})$ for the
remainder of the sweep.  The value $p_{t'}$ is a deletion event.

Note that \lemref{monotone} also allows for binary search on the value
...
\begin{proof}
  ...
  to be defined later.  Label the elements of $\Delta$ as $T_1,\ldots,T_n$
  ...
  $T''$ such that (a)~the size of $T''$ is equal to the size of $T_i$ and
  ...
  contradiction to the assumption that $|A(\Delta)| > rn$.  We conclude that
  $|A(\Delta)| = O(nk^{d-1})$, as required.
\end{proof}
...
Thus, a version of this lemma holds for collections of \emph{fat} homothets,
...
\begin{proof}
  ...
  \begin{figure}
    \begin{center}
      \begin{tabular}{ccc}
        \includegraphics{construction-a} & 
        \includegraphics{construction-b} & 
        \includegraphics{construction-c} \\
        The simplex $V$ & 
        $T$ intersects $T'$ & 
