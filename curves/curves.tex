...
Now, consider $v_1'=\frac{1}{k}\sum_{v\in S} v_1$.  Then we have
\begin{eqnarray*}
 & & \sum_{i=1}^m \cos\left(\frac{\pi i}{m}\right)
   \left(\frac{|h_{\pi i/m}\cap S|}{k} -
   \frac{|h_{\pi (i+1)/m}\cap S|}{k}\right) \\
 & \le & \sum_{i=1}^m \cos\left(\frac{\pi i}{m}\right)
   \left(\ccap\left(\frac{\pi i}{m}\right) -
   \ccap\left(\frac{\pi (i+1)}{m}\right) + \frac{2}{r}\right) \\
 & \le & c_d + \frac{2m}{r} + \ldots
\end{eqnarray*}
and 
\[
 v_1'\le \sum_{i=1}^m \cos\left(\frac{\pi i}{m}\right)
   \left(|h_{\pi i/m}\cap S| -
   |h_{\pi (i+1)/m}\cap S|\right) 
\]


\comment{
Lukovszky \cite{tamasthesis} gives a proof that, for any $\eps>0$, and
any $d>1$, there exists a centrally symmetric $d$-polytope,
$P_{d,\eps}$, all of whose vertices on the unit sphere
$\mathbb{S}^{d-1}$ and having $O((1/\eps)^d)$ facets, all of which
have angular diameter at most $\eps$.  That is, for any pair of points
$p$ and $q$ in some facet $f$ of the polytope $\angle pOq\le \eps$.

Observe that, for any ray $r$ originating at the origin, the
intersection of $r$ with the boundary of $P_{d,\eps}$ is of distance
at most $\eps$ from the intersection of $r$ with
$\mathbb{S}^{d-1}$.\marginpar{Why?}  In our setting this means that,
if we can define a distance function where is the unit ball is
$P_{d,\eps}$ then this distance function is an $\eps$-approximation to
Euclidean distance.  Indeed, we will show how to choose the lines
$\ell_1,\ldots,\ell_k$ and the weights $w_1,\ldots,w_k$ in such a way
that the set of points $p$ that satisfy
\[
 \frac{c_d}{dk} \sum_{i=1}^k w_i|p_{\ell_i}-q_{\ell_i}| \le 1
\]
is exactly the the polyhedron $P_{d,\eps}$.  For the lines
$\ell_1,\ldots,\ell_k$, we take one line perpendicular to each pair of
antipodal faces in $P_{d,\eps}$.
}
