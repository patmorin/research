...
\begin{algorithmic}
\FOR{each node $j$ on the rightmost path of $T$}
   \FOR{$\ell=a,\ldots,b-\mathrm{value}(j)$}
      \STATE{delete $p_{i,\ell},\ldots,p_{i,\ell+\mathrm{value}(j)}$ 
          from $S_i$, exposing $p_{i+1,a'},\ldots,p_{i+1,b'}$ of
             $S_{i+1}$}
      \STATE{$X_{j,k}=\textsc{FindValue}(\mathrm{left}(j),)$}
   \ENDFOR
\ENDFOR
\end{algorithmic}
