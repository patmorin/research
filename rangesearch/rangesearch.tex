...
\emph{split} if $v$ is not bad, and $r(v)\cap S\neq \emptyset$.  The
two children of a split node $v$ are associated with the two regions
obtained by removing a horizontal or vertical strip $s(v)$ from $r(v)$
...
node}, otherwise we call $v$ a \emph{horizontal node}. 
...
$\lft(v)$ and $\rght(v)$ depending on whether the child is below or
above the strip, respectively.  Similarly, the left and right
...
that each of the at most two components of $r(v)\setminus s(v)$ has
...
and $r(\lft(v))$ are the two components of $r(v)\setminus s(v)$.  This
implies that $r(\lft(v))$ and/or $r(\rght(v))$ may be empty, in which
case $\lft(v)$, respectively, $\rght(v)$ is a leaf of $T$.  With these
definitions, for any point $q\in \R^2$ there is exactly one vertex $v$
of $T$ such that $q\in s(v)$.
...
\item Any node $v$ at depth $i$ in $T$ has $\Pr(s(v))\le \Pr(r(v))\le 1/2^i$ (by
the choice of $s(v)$).

\item For any node $v$ of $T$, if $\Pr(r(v)) > 0$, then the closure of
$r(v)$ contains at least one point of $S$. (Either $r(v)$ contains a
point of $S$, or $r(v)=\emptyset$ (so $\Pr(r(v))=0$), or the left or
bottom side of $r(v)$ contains a point of $S$.)
...
catalogues of $v_1,\ldots,v_k$.  However, performing these queries
individually would take too long.

To speed up the process of navigating the catalogues of $T$,
fractional cascading \cite{ae76} is used.  Refer to
\figref{cascading}. If $v$ is a horizontal
...
drawn according to the distribution $D_{\mid s(\prnt(v))}$.
Otherwise ($v$ is not a left child), the weight of an interval is determined by the distribution
$D_{\mid s(v)}$.
...
$q$ in the catalogues of $u$.  We will show that each of these
