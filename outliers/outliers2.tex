...
The problem of finding a subset of $S$ of size $k$ with minimum
diameter has been studied by Aggarwal \etal\ \cite{aiks89} who give an
$O(k^{2.5}n\log k+n\log n)$ time algorithm.  The same authors give an
$O(k^2n\log n)$ time algorithm to find the the subset of $S$ with the
minimum perimeter enclosing axes-parallel rectangle.
