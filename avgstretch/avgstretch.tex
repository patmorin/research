...
\emph{hub}, $u_i$ (see \figref{overview}). Let $H=\{u_1,\ldots,u_{n'}\}$
denote the set of hubs and recall that $|H|=n'\in O(n/k)$.
...
Our graph $G$ starts with two spanner constructions.  The first spanner,
denoted $G_2=(V,E_2)$, is a 2-spanner of $V$ and has $O(n)$ edges
\cite{callahan.kosaraju:faster,salowe:constructing,vaidya:sparse}.
The next spanner, denoted $G_1=(H,E_1)$, is a
$(1+1/k^{1/(d-1)})$-spanner of $H$ and has $O(k|H|)=O(n)$ edges
\cite{carmi.smid:optimal,ruppert.seidel:approximating}.

With these two spanners we have, for any $u,w\in V$
\[
...
\begin{figure}
  ...
  \end{tabular}
  \caption{Large cities have many direct routes to them. Source: Map of the final U.S. Highway system as approved November 11, 1926.}
  \figlabel{us-map}
\end{figure}
