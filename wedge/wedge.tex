...
We then generalize the bounds of \thmref{twod-result} to point sets in
...
\section{Proof of \thmref{dd-result}}

In this section, we prove \thmref{dd-result}.  Most of the proof is
identical to the proof of \thmref{twod-result}.  In particular, the
following bounds come from exactly the same arguments used in the
proofs of \lemref{a}, \lemref{b} and \lemref{c}.

\begin{lem}\lemlabel{dd-a}
...
$f^d_\alpha(n) \le \ceil{n/(d+1)}$ for $\alpha < \theta_d$.
\end{lem}
...
$\theta_d$-wedge that contains $p$ and whose interior intersects only
one of the rays.

Let $C_1,\ldots,C_{d+1}$ be the closed cones obtained by taking the
affine hull of each subset of $d$ vertices of $T$.  Notice that these
cones cover $\mathbb{R}^d$ and that each cone contains $d$ of the
$d+1$ rays that contain $S$.  Furthermore, if the cone $C_i$ contains
the point $-p$ then, by \lemref{cone-angle}, there is a
$(2\pi-\theta_d)$-wedge
whose apex is at $p$ and that contains $C_i$.  Thus the interior of
the complementary $\theta_d$-wedge does not intersect $C_i$ and hence
intersects only one of the $d+1$ rays that contain $S$. This
$\theta_d$-wedge contains an $\alpha$-wedge that contains $p$ and
contains at most $\ceil{n/(d+1)}$ points of $S$, as required.
\end{proof}

\begin{lem}\lemlabel{dd-c}
 $f^d_\alpha(n) \ge \ceil{n/2}$ for $\alpha \ge \theta_d$.
\end{lem}
...
$D_\alpha^d(p,S)\ge \ceil{n/2}$. To see this, observe that \lemref{any
$(2\pi-\theta_d)$-wedge contains at most 
...
\begin{proof}[Proof of \thmref{dd-result}]
The theorem follows immediately from \lemref{dd-a},
\lemref{dd-b} and \lemref{dd-c}.
\end{proof}
