...
\begin{rem}\remlabel{epsilon-choice}
   ...
   $\epsilon \in\Omega((k/n)^{1/3})$. 

   It will be particularly useful to fix $k=(n/\log n)^{(d-1)/(4d-1)}$,
   and set $\epsilon = (\log^\kappa n)/k$, for some constant $\kappa
   \ge 0$.  It is then straightforward to verify that the requirement
   $\epsilon \in \Omega((k/n)^{1/3})$ is satisified for any constant
   $d\ge 2$.  Furthermore, for all $d \ge 3$,
   \[
