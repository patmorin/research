...
markets are considered.  In the model considered, a product is defined by
$d$ real-valued qualities whose sum determines the unit cost of producing
the product.  The optimization problem considers the following scenario:
given a set of $n$ customers, each of whom has certain minimum quality
requirements and a maximum price they are willing to pay, the goal is to
design a new product and select a price for that product in order to
maximize the resulting profit.

An $O(n\log n)$ time algorithm is given for the case, $d=1$, of linear
products, and $O(n(\log n)^{O(d)})$ time approximation algorithms are given
for products with any constant number, $d$, of qualities.
\end{abstract}
...
requirements. From the point of view of a manufacturer introducing one or
more new products, this means that all customers are \emph{Pareto optimal},
i.e., there are no two customers $C_i$ and $C_j$ such that $q_{i,k} >
q_{j,k}$ for all $k\in\{1,\ldots,d\}$ and $p_i < p_j$.  This is because, in
a saturated market, $C_j$ and $C_i$ will purchase the same product, namely
the lowest-priced product that satisfies $C_i$'s (and therefore also
$C_j$'s) requirements.  When modelling a saturated market, there is no need
to explicitly consider existing products since these can be encoded into
...
given a \emph{market} of customers $M=\{C_1,\ldots,C_n\}$, each have $d$
qualities, the $\textsc{ProductDesign}(d)$ problem is to find a product
$P^*\in\R^{d+1}$ such that
\[
  \val(P^*) = \ppu(P^*)
    \times 
...
$\textsc{ProductDesign}(1)$ (\secref{1-d}), and $O(n(\log n)^{O(d)})$ time
approximation algorithms for $\textsc{ProductDesign}(d)$ (\secref{2-d}
...
\begin{proof}
  ...
  occurs $2$ or more times, then the product $(x,x+1/2)$ gives a value
  $\val(x,x+1/2) \ge 1$.  On the other hand, if there is no such $x$,
  then \begin{enumerate} \item any product $(q,p)$ with $p-q>1/2$ can not
  be sold to any customers and \item any product $(q,p)$ with $p-q>0$
  can be sold to at most $1$ customer.  \end{enumerate} Therefore,
  if all the elements of $A$ are unique, then $\val(q^*,p^*) = 1/2$,
  otherwise $\val(q^*,p^*) \ge 1$.  The result follows.
\end{proof}
...
solution to  $\textsc{ProductDesign}(2)$ can be reduced to a sequence
of problems on the planes $H_0,\ldots,H_\ell$.  In fact, each customer
$C_j$ defines a (possibly empty) equilateral triangle $\Delta_{i,j}$
on the plane $H_i$ such that $C_j$ will consider a product $P$ in $H_i$
if and only $P$ is in $\Delta_{i,j}$.  Thus, the problem of solving
$\textsc{ProductDesign}(2,1)$ restricted to the plane $H_i$ is the
problem of finding a point contained in the largest number of equilateral
triangles from the set $\Delta_i=\{\Delta_{i,j}: j\in\{1,\ldots,n\}\}$.
...
\begin{thm}
  For any $\eps >0$, there exists an $O(\eps^{-3}n(\log n)^2)$
  time Monte-Carlo $(1-\eps)$-approximation algorithm for
...
Again, each customer $C_j$ defines a regular simplex $\Delta_{i,j}$ in
$H_{i}$ such that $C_j$ will purchase $P\in H_i$ if and only if
$P\in\Delta_{i,j}$.  In this way, we obtain a set
$\Delta_i=\{\Delta_{i,1},\ldots,\Delta_{i,n}\}$ of homothets of a regular
simplex in $\R^d$ and we require an algorithm to find a
...
\cite{ah08} can be used to help solve this problem, but not before we prove
some preliminary results, the first of which is a combinatorial geometry
result.  
...
\paragraph{Remark.} The proof of \lemref{arrangement} makes almost no use
...
\begin{proof}
  ...
  over all $n$ queries will be be the $O(|A(C)|)=O(nk^{d-1})$.
...
\begin{cor}\corlabel{deep-point}
  ...
  of $\Delta$.  Then there exists a Monte-Carlo algorithm whose running
  time is $O(\eps^{-2d}n(\log n)^{d-1} + n(\log n)^d)$ and that, with high
  probability, returns a point $p'\in\R^{d}$ contained in at least $(1-\eps)\delta$
  elements of $\Delta$.
\end{cor}
...
\begin{thm}
  For any $\eps >0$, there exists an $O(\eps^{-(2d+1)}n(\log n)^d +
  n(\log n)^{d+1})$ time Monte-Carlo $(1-\eps)$-approximation algorithm
  for $\textsc{ProductDesign}(d)$.
\end{thm}
...
$\textsc{ProductDesign}(1)$ and $O(n(\log n)^O(d))$ time approximation
...
that produces a $(2-\eps)$-approximation algorithm can run in $o(n\log n)$
time.
...
is known.  Is it possible possible to prove some kind of a lower bound?
The related problem of finding the point contained in the largest number of
unit disks is \textsc{3-Sum} hard providing some evidence that this problem
...
some number, $k$, $k>1$, of new products into a market.  For the case
$d=2$, this already appears to be NP-hard.  The sketch of this NP-hardness
proof is as follows:  Let $X$ be a set of $n$ identical equilateral
triangles such that, for any two triangles in $T_1,T_2\in X$, $T_1\cap T_2$
is either empty of is contained in a ball of radius $\delta$ around one of
$T_1$'s vertices.  Determining whether there exists a set $S$ of $n/2$
points such that every triangle in $X$ contains a point in $S$ can be shown
to be NP-hard using a proof similar (but a little simpler) than the proof
by X and Y that vertex cover in penny graphs is NP-hard.  Given the set $X$
described a above, we can produce a set of customers such that there exists
$k$ products whose 

 since 
using a construction similar to that of \secref{X} it is possible to
take


\secref{X}, a solution
problem can be used to solve the problem of finding the fewest number of
points that \emph{stab} \cite{X} a set of equilateral triangles, which is
probably NP-hard \cite{X,X,X}.

Does the case $d=1$ have a polynomial time solution?  Does the case $d=2$
have a polynomial time approximation algorithm?  Despite some effort, we
have not been able to find algorithms, approximate, or exact, that avoid
having $k$ in the exponent of the running time. 
