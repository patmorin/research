...
\thmref{martingale} on the sequence $Q_i=\sum_{j=1}^i(X_j-\E[X])$ we
...
\end{cor}
In plain english, \corref{hitting-time} says that, at least half the
time, at some point
...
$Q_1,Q_2,\ldots$ so, by the \thmref{martingale}
\begin{equation}
  \E[Q_T] 
    = \E[Q_1] 
...
\[
  \E\left[\sum_{i=1}^T R_i\right] 
   ...
   \enspace ,
\] 
where the last two inequalities follows from \lemref{reset}.
\end{proof}
...
well-behaved.  In this case, $\mathcal{A}$'s FSM contains a set
of \emph{terminal components}.  These are disjoint sets of vertices of the
FSM that are strongly connected and that have no outgoing edges (edges
with source in the component and target outside the component).
