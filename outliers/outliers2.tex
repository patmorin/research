...
% This is to override elsart paragraph numbering
\renewcommand{\paragraph}[1]{\noindent\textbf{#1}\ }

\begin{document}
\begin{frontmatter}

\title{Algorithms for Optimal Outlier Removal\thanksref{funding}}
...
time, provided $\frac{n}{2}<k\le n$. Their algorithm can also find the
...
except to this is the algorithm of Kedem and Segal \cite{sk98} which
...
NP-hard problems \cite{df99}.  In particular, our solution to Problems
...
problem.  \secref{box} presents algorithms for the (bounding box)
...
\FOR{$g=1$ to $c+1$}
  ...
  \STATE{$x\gets$ $(c+1)$-st largest entry in $A_{\star,j}$}
  \STATE{$\tilde{T}\gets 
...
Note that the proof of \lemref{catalan} shows how to convert a binary
...
\STATE{$N\gets$ the sum of node labels on rightmost path of $T$}
\IF{$k-j < N + d$}
  \RETURN{$-\infty$ \COMMENT{* No solution possible *}}
\ENDIF
\FOR{$g=1$ to $d$}
...
$p_{i,j}$ is deleted (in line 9) by the algorithm.  All other work
