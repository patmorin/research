...
\footnotetext[1]{
School of Computer Science, Carleton University, Ottawa, Canada. Email: \texttt{\{jit, morin, vida\}@scs.carleton.ca}. Research partially supported by NSERC.}


\footnotetext[2]{
Departament de Matem\`atica Aplicada II, Universitat Polit\`ecnica de Catalunya (UPC), Barcelona, Spain. Email: \texttt{Ferran.Hurtado@upc.edu}. Research partially supported by Projects MCYT-FEDER BFM2003-00368 and Gen. Cat 2001SGR00224.}
...
(\figref{fe-two}.f). Otherwise, if there is a path through
...
Then $\W\W(p)$ cannot be black (\figref{fe4}.a), otherwise there is
...
through  $\SW(p)$ (\figref{ef-two}.d) or every path from $g$ to $p$ in
$B_8(I)$ goes through $\N(p)$ (\figref{ef-two}.e). Otherwise,
...
(\figref{ef-twoI}.a). Thus assume that at least one of
$\{\N(z),\N\N(z)\}$ is black. 
...
connectivity  (\figref{ef-twoI}.b). Therefore, $z\in \W\S\S\S\S^+(l)
\cap \N^*(g)$.
...
white) (as an example consider (\figref{ef-twoI}.e)). In any case,
$W_4(I^4)$ is connected since it can be obtained by adding $z$ to
$W_4(I^3)\sm \NE(z)$ where $z$ is adjacent in $W_4(I^4)$ to $\E(z)$
and $\N(z)$ (if white). $B_8(I^4)$ is connected by \obsref{cut-vertex}
...
Similarly, $\SW(p)$ has to be black (\figref{ee-two}.a) as otherwise,
...
Assume first that $\NW(p)$ is black. (\figref{ee-two}.b). Then
$\W\W(p)$ has to be white (\figref{ee-two}.c) as otherwise $p$ is not
...
has only two such neighbours in $B_8(I^1)$ (\figref{ee-two}.c), namely
...
black (\figref{ee-two}.d). If $\W(p)$ is not a cut vertex of $W_8(I)$
...
$\SW(p)$ that does not go through $p$ (\figref{ee-two}.e). We claim
...
$\NW\W(p)$ is black, $\N\NW(p)$ has to be  white (\figref{ee-two}.f)
...
\N\W\W(p)$ (\figref{ee-two}.f). That however implies again a path in
...
is a cut elbow in $I$. 
%We claim that $I$ admits a $k$-diagonal interchange at $p$ with the
%desired properties. 
...
resulting image $I^1$. By \obsref{cut-vertex}, $B_4(I^1)$ is connected
...
Applying (a diagonal interchange of) \lemref{fourfour1} to
...
applying \lemref{fourfour1} to $\N\NW(t_{I_2})$) until we arrive at an
...
maintained. Furthermore, diagonal interchanges of \lemref{fourfour1}
...
(a diagonal interchange of) \lemref{fourfour1} to $q$ results in an
...
Now apply (a diagonal interchange of) \lemref{fourfour1} to $p$. None
...
from consecutive applications of \lemref{fourfour2}, such that each
image in the sequence has the same width and the same number of elbows
in their frontier. By \lemref{fourfour2} each image $I_i$, $1<i\leq s$
...
Thus applying \lemref{fourfour2} to $I_s$ results in an image
