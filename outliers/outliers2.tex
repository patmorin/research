...
the running time to $O(n\log n + k^2 n)$ using $O(kn)$ space. Datta
\etal\ improve the space bound to $O(n)$. Segal and Kedem \cite{sk98}
...
\cite{e93} who give $O(kn^{3})$ and $O(n^2\log n + k^3n^2)$ time
algorithms for this problem, respectively. \comment{The second algorithm (by
...
\FOR{$g=1$ to $c+1$}
  ...
  \STATE{$x\gets$ $(c+1)$-st largest entry in $A_{i,*}$}
  \STATE{$\tilde{T}\gets 
     \tilde{T} \cup \{ k : A_{i,k} \ge x \}$ 
          \COMMENT{* mark $c+1$ largest entries in row $i$ *}}
  \STATE{$x\gets$ $(c+1)$-st largest entry in $A_{j,*}$}
  \STATE{$\tilde{T}\gets 
     \tilde{T} \cup \{ k : A_{k,j} \ge x \}$
          \COMMENT{* mark $c+1$ largest entries in column $j$ *}}
...
\begin{lem}\lemlabel{diameter_thm}
...
Carefully inspecting the proof of \lemref{diameter_thm} shows that it
