...
Throughot this section, we say that an event within an algorithm happens
...
constant factor.  When describing algorithms in this section, we will
use the variable $\alpha$ to describe certain aspects of the algorithm.
...
\begin{proof}
  ...
  n/n)^{1/(4d-1)})$.  (A key step occurs in the analysis of Type~4 pairs,
  where we must check that the new value of $\epsilon$ still satisfies
  $\epsilon \ge D\sqrt{k/n}$---which it does.) What remains is to show
  how the graph $G'$ can be constructed in $O(n\log^d n)$ time.
  ...
  can, for any $\gamma >0$, construct a $(1+\gamma)$-spanner of $n'$ points
  in $\R^d$ that has $O((1/\gamma)^{d-1}n')$ edges in $O((1/\gamma)^{d-1}
  n'\log n')$ time.  Using this algorithm with $\gamma=1/k^{1/(d-1)}$,
  ...
  Our first task is to find the boxes $E_1,\ldots,E_{n'}$ and for this we
  use random sampling.  Let $D'_{i}$ be the square box that is centered
  at the center of $\Delta_i$ and has side-length $(c+1/2c)L(\Delta_i)$.
  We repeatedly select a random point $u\in V\cap D'_{i}$ and count the
  ...
  number of points is chosen as $E_i$.
...
provided that we are willing to sacrifice a little inside the $o_n(1)$ term
...
\begin{proof}
  ...
  We address each of the preceding three problems in turn. Doing so
  requires making some changes to the algorithm for constructing $G'$
  that improve the running time but also affect the stretch factor.
  We will first establish that these changes do, indeed, reduce the
  running time to $O(n\log n)$.  Afterwards we will discuss what affect
  these changes have on the stretch factor.
  ...
  \begin{enumerate}
     ...
     accurately enough estimate of $|B\cap V|$.
     ...
     and counting can be done in $O(n\log n)$ time using $T_1$.

     We note that the sample obtained this way is a random sample of
     $V'$, which is itself a Bernoulli sample of $V$. Despite this, the
     sample will still, with high probability, sample a point in the
     box $E'_i$ described in the proof of \thmref{algorithm}.  This can
     be proven by first using Chernoff's Bounds to show that, with high
     probability, the number of points in $E'_i\cap V$  that are in the
     sample $V'$ is at least $p\epsilon n/2$ and then proceeding as in
     the proof of \thmref{algorithm} to show that, with high probability,
     at least one of these points is sampled.
    ...
    \item To determine the index, $i(u)$, of points $u\in V$, we
      ...
      From the discussion, $T_2$ then contains only boxes, $B$ such
      that $|B\cap V|\ge \epsilon n/2$ and $T_2$ includes every box,
      $B$ such that $|B\cap V|\ge 2\epsilon n$.

      Determining which boxes to include in $T_2$ requires $n'$ queries
      in $T_1$, so this takes $O(n'\log^{d-1} n)\subset O(n\log n)$
      time.  Building the tree $T_2$ takes $O(n'\log^{d} n)\subset
      O(n\log n)$ time.  At this point, we would like to use $T_2$
      to compute $i(w)$ for every point $w\in V$, but this would take
      $\Omega(n\log^d n)$ time.  Instead, we take a sample, $V''\subseteq
      V$ of size $n/\log^{d-1} n$ and compute $i(w)$ for each $w\in V''$.
      ...
      proof of \thmref{upper-bound}.

      Recall that this part of the proof of \thmref{upper-bound} assumes
      that that number of bad Type~4 pairs is at least $\epsilon
  ...
  \end{enumerate}
  Finally, it remains to establish the effect of the preceding changes
  on the stretch factor of the resulting graph.  As it turns out, the
  only change that makes any difference to the spanning ratio is the
  change to the spanner of $N$.
