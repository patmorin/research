...
Thus, there exists very sparse graphs with constant average stretch
factor.
...
\begin{rem}\remlabel{epsilon-choice}
   ...
   $\epsilon \in\Omega(k/n)$.  Observe that, if we fix $k=(n/\log
...
\begin{proof}
  ...
  $O((1/\gamma)^{d-1} n'\log n')$ time.  Using this algorithm with
  $\gamma=1/k^{1/(d-1)}$, the $(1+1/k^{1/(d-1)})$-spanner of the $n'$
  points in $N$ can be constructed in time
  \[
