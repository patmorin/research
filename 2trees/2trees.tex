...
The inductive proof in \lemref{if} results in a recursive algorithm that
...
The base case of \lemref{basecase-12} does not necessarily have a
run-length encoding of constant-size, but can be checked in constant
time by checking if $\Delta_k = n-1$.  \lemref{basecase-13} also does
not necessarily have a run-length encoding of constant size but it can
be verified that, if the conditions of \lemref{basecase-13} hold then
they hold with $z$ being selected from among the three largest values
in $D$.  Thus, the conditions of \lemref{basecase-13} checked in
