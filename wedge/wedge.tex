...
\lemref{a}, \lemref{b} and \lemref{c}.
...
concurrent halving lines used in \lemref{construction}, should yield a
much better result.

\begin{lem}\lemlabel{borsuk}
Let $V=\{v_1,\ldots,v_{\nov}\}$ be any set of unit vectors in
...
a set $H=\{h_1,\ldots,h_{\nov}\}$ of halving hyperplanes of $S$ that
all contain a common point $p$ and such that $h_i$ is perpendicular
to $\rho(v_i)$, for all $i\in \{1,\ldots,\nov\}$.  
\end{lem}

\begin{proof}
For a correct proof, we have to embed the sphere $S^{d(d-1)/2-1}$ into
the space $SO(d,\mathbb{R})$ of all rotations of $\mathbb{R}^d$ so
that antipodal points $x$ and $-x$ on the sphere map to rotations 
that are the inverse of each other.
See the next lemma for how to do this in $\mathbb{R}^3$.
\comment{
The proof is an application of the Borsuk-Ulam Theorem.  The group of
all rotations of $\mathbb{R}^d$, denoted $SO(d,\mathbb{R})$ is a lie
group of dimension $d(d-1)/2$.  The sphere $S^{d(d-1)/2-1}$ is a real
manifold that can be embedded into a small portion of
$\mathbb{R}^d(d-1)/2$ and therefore can be embedded into
$SO(d,\mathbb{R})$ by some function $f:S^{d(d-1)/2-1}\rightarrow
SO(d,\mathbb{R})$.  (In words, each point on $S^{d(d-1)/2-1}$ is being
mapped onto some rotation of $\mathbb{R}^d$, with most rotations
having no preimage.)

Notice that, for any rotation $\rho$ of $V$ there exists $\nov$
oriented halving hyperlanes of $S$ that are perpendicular to the
elements of $\rho(V)$, but these hyperplanes may have an empty common
intersection.  Nevertheless, we call this set of hyperplanes
$H^\rho=\{h^\rho_1,\ldots,h^\rho_{\nov}\}$.  Notice that, as $\rho$
changes continuously, the elements of $H^\rho$ also change
continuously.  Let $p^\rho$ denote the point of common intersection of
$h^\rho_1,\ldots,h^\rho_d$, and define the $(d(d-1)/2-1)$-vector
$X(\rho)$ whose $i$th entry is the \emph{signed} distance of the
hyperplane $h^\rho_{d+i}$ to the point $p^\rho$.  Now, notice that $X$
is a continuous mapping of $SO(d,\mathbb{R})$ (the values of $\rho$)
onto $\mathbb{R}^{d(d-1)/2-1}$ (the values of $X$).

Furthermore, this mapping is antipodal, i.e. $X(-\rho) = -X(\rho)$.
Therefore, by the second version of the Borsuk-Ulam Theorem
\cite[Theorem~2.1.1, version BU1b]{mXX}, there exists a rotation
$\rho$ such that every element of $X(\rho)$ is zero, i.e., all the
hyperplanes in $H^\rho$ contain the point $p^\rho$.  This completes
...
\begin{proof}
The upper bound appears already in \lemref{combined}.  For the lower
bound, consider the rotation $\rho$ and the set
$H=\{h_1,\ldots,h_{\nov}\}$ of halving hyperplanes we obtain by
applying \lemref{borsuk} to the set $V=\{v_1,\ldots,v_{\nov}\}$ of
vectors in \lemref{vectors}.  Take the common intersection point $p$
