...
each has at most $k|S|$ edges.  They are \emph{spanners}; the length
...
a spanner for any $k\ge 4$ \cite{barba.bose.ea:on,bose.morin.ea:theta-5,%
keil:approximating,ruppert.seidel:approximating}.
...
fewer than $k$ edges; the number unoriented $\theta$-maxima is typically
...
so it should not be counted twice.
...
We begin, in \secref{poisson} by studying (the infinite) $\theta_k$-graphs
generate by a homogeneous Poisson point process with unit intensity
over the entire Euclidean plane.  In this model, we study the average
degree of a vertex in the $\theta_k$-graph.  For the $\theta_k$-graph,
this quantity is at most $2k$ since each vertex defines $k$ edges of
the graph and each edge has 2 endpoints.  However, in some cases an edge
$uw$ is \emph{mutual} in the sense that the edge is created both by $u$
and by $w$.  If we let $p_k$ denote the probability that an edge of the
$\theta_k$ graph is is mutual, then the average degree of the $\theta_k$
graph is
\[
    d_k = (2-p_k)k \enspace .
\]
...
theta graphs and their applications, though most of this work focuses
on worst-case analysis.

One notable work is that of Devroye \etal\
...
n)$.\footnote{Devroye \etal\ actually consider closely-related Yao graphs
\cite{flinchbaugh.jones:strong,yao:on}, although their proofs apply,
...
sets have been studied extensively.  Devroye \cite{devroye:expected}
...
graphs.  Devroye \cite[Section~7]{devroye:expected} also mentions
``directional nearest-neighbour graphs,'' now commonly known as Yao
graphs \cite{flinchbaugh.jones:strong,yao:on}, and points out that his
theorem does not apply to these (nor does it apply to theta graphs).
...
The issues of connectivity and giant components in the
$r$-disk graph---in which an edge $uw$ is present if and only if the
distance between $u$ and $w$ is at most $r$---is the subject of
...
In the Poisson model, the number of points in any region whose area is $A$
...
\begin{proof}
  ...
  \]
  where $\alpha=\ell^2\cos(\theta/2)/(4\sin(\theta/2))$.  We now have
...
Next, we determine the value of $p_k$ for odd values of $k\ge 5$.
...
$X\subseteq [0,\sqrt{n}]^2$ whose area is $A$ does not contain any points
of $S$ is exactly $(1-A/n)^n=\exp(-A)-O(1/n)$.  This is consistent with
the Poisson model up to an additive error of $O(1/n)$.
...
lower-order error term.  We will use the notation $x= y\pm a$ denotes
that $x$ is some value in the interval $[y-a,y+a]$.  We will abuse this
notation slightly by writing equations of the form $x\pm a = y\pm b$
when $[x-a,x+a]\subseteq[y-a,y+a]$.
...
\begin{align}
   \Pr\{\mui\mid\eui,\,\ell,\, r\}
        ...
        & = \exp(-\alpha(r^2+(\ell-r)^2))
            \pm O((\alpha^3\ell^5+\alpha\ell^2)/n) \notag \\
        & = \exp(-\alpha(r^2+(\ell-r)^2))
            \pm O((\alpha^3 t^5+\alpha t^2)/n) \enspace . \notag
\end{align}
...
\begin{align*}
   f'(\ell) \equiv & \Pr\{\mui\mid\eui,\,\ell\} \\
     & = \int_0^\ell(1/\ell)\left(\exp(-\alpha(r^2+(\ell-r)^2)) 
           \pm O((\alpha^3 t^5+\alpha t^2)/n)\right)\,\mathrm{d}r \\
     & = f(\ell) \pm O((\alpha^3 t^5+\alpha t^2)/n) \enspace ,
\end{align*}
...
\begin{align*}
  p'(x) & \equiv \frac{d}{dx}P(x) \\
        ...
        & = 2\alpha x\exp(-\alpha x^2)\pm O((1+\alpha^2 x^3)/n) \\
...
\begin{align*}
    \Pr\{\mui \mid u\in C\} 
         ...
         & = (1-n^{-\Omega(c)})\int_0^t (p(\ell)\pm O(\alpha^2t^3/n))\cdot(f(\ell)\pm O((\alpha^3 t^5+\alpha t^2)/n))\,\mathrm{d}{\ell} \\
         & = (1-n^{-\Omega(c)})\int_0^t p(\ell)f(\ell)
           \pm O(p(\ell)(\alpha^3t^5+\alpha t^2)/n + f(\ell)\alpha^2t^3/n + 1/n)
            \,\mathrm{d}{\ell} \\
        & = \frac{\pi\sqrt{3}}{9} \pm O((\alpha^3t^5+\alpha t^2+\alpha^2t^4 + 1)/n) \\
        & =  \frac{\pi\sqrt{3}}{9} \pm O((\log n)^\frac{5}{2}/n)
\end{align*}
and
\begin{align*}
    \Pr\{\mui \mid u\in C\} 
       ...
       & \le \Pr\{\eui\mid u\in C\}\cdot \Pr\{\mui\mid \eui\} 
           + n^{-\Omega(c)} \\
         & = \frac{\pi\sqrt{3}}{9} \pm O((\log n)^\frac{5}{2}/n) \enspace . \qedhere
\end{align*} 
\end{proof}
   ...
   \begin{center}
     \begin{tabular}{r|r|r|r|r|}
       \multicolumn{1}{c|}{$k$} & \multicolumn{1}{c|}{$p_k$} & \multicolumn{1}{c|}{$d_k$} & \multicolumn{1}{c|}{$d_k/k$} & \multicolumn{1}{c|}{$\hat{d}_k$} \\ \hline
        ...
        20 & 0.64646902 & 27.90800424 & 1.39540021 & 27.91343314  
        %\hline
     \end{tabular}
   \end{center}
...
also shows the results of the following experiment: 4,000 points were
generated uniformly in the unit square $[0,1]^2$ and their $\theta_k$
graph was computed.  The average degree of points in the square
$[1/3,2/3]^2$ was then computed.  The final column of \tabref{values}
shows the average of these values taken over all 2,000 repetitions of
...
\]
for which we are unable to obtain a closed form.  Nevertheless, one can
evaluate this integral numerically to obtain estimates of the mutual
edge probability and average degree in Yao graphs.
