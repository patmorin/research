...
\begin{abstract}
  ...
  average stretch factor of this graph is $1+O((\log^2 n/n)^{1/(2d+1)})$.
...
More precisely,
\[
   \asf(G)=1+O((\log^2 n/n)^{1/(2d+1)}) \enspace .
\]
The proof of this result is in \secref{upper-bound} and constitutes the
bulk of the paper.  

In \secref{algorithms}, we also show that graphs with small average
stretch factor can be constructed efficiently.
FIXXXXXXXXXXXXXXXMMMMMMMMMMMMMMMMEEEEEEEEEEEEEEEEEEEEEE
...
\begin{proof}
  ...
  \[
    \delta n^2 \in O(k^{1/2}n^{3/2}c^{d/2}\log c) \enspace .
  \]
...
%  But then Property~4 of $k$-partitions implies that $\mu\in O(1)$.
  ...
  \begin{enumerate}
    ...
    \item Category B: $w\not\in E_{i_{j^*}}$.
      ...
      is $O(tk/\epsilon)$.  Each such point, $w$, forms a bad pair
      with at most $tk$ different values of $u$---namely the elements
      of $V_{i_{j^*}+1},\ldots,V_{i_{j^*+t}}$. Therefore, the number of
  ...
  $j\in\{j^*+1,\ldots,j^*+t\}$.  Since there is an edge joining $u$
  and $w_{i_{j}}$, it is sufficient to prove that $\|w^*w_{i_{j}}\|\in
  ...
  $r_{i'}\le r_{i_{j^*}}$, and $w_{i_{j}}\in E_{i'}$.
  ...
  \begin{enumerate}
    ...
    define at least one pair, this implies that $\Delta_{i_j}$ is
    ...
    large constant, $C$, $r_{i_{j^*+t+C}} > 2r_{i_{j^*+t}}$ and $E'\subset
    E_{i_{j^*+t+C}}$.
  ...
  \begin{equation}\eqlabel{master-asf}
     \asf(G) = 1 + O\left(1/k^{1/(d-1)} + 1/c 
       + \binom{n}{2}^{-1}\left(kn+k^{1/2}n^{3/2}c^{d/2}\log c
  ...
  \]
  \[ 
       k = \left(\frac{n}{\log^2 n}\right)^{(d-1)/(2d+1)}
  \]
  and
  \[
       c = \left(\frac{n}{\log^2 n}\right)^{1/(2d+1)}
  \]
  yields
  \[
     \asf(G) = 1+ O\left((\log^2 n/n)^{1/(2d+1)}\right) \enspace .
  \]
\end{proof}
...
\begin{rem}\remlabel{epsilon-choice}
   ...
   useful when designing algorithms in the next section.  The choice of
   the value $\epsilon$ remains free up until the last step in the proof
   of \thmref{upper-bound}, but is subject to the constraint $\epsilon
   \in\Omega((k/n)^{1/3})$.  Observe that, if we fix $k=n^{(d-1)/(4d-1)}$
   and set $\epsilon = (\log^\kappa n)/k$, for
   any constant $\kappa \ge 0$, then the requirement $\epsilon \in
   \Omega(\sqrt{k/n})$ is satisfied for any $d\ge 2$.  Furthermore,
   for all $d \ge 3$,
   \[
