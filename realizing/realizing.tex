...
study of the perception of musical melody and rhythm.  Experimental
evidence shows that humans can recognize tunes given only the
melodic-contour, i.e., the sequence that specifies whether one note is
at a higher, lower, or the same pitch as the previous note.  It then
becomes a natural question to ask whether a given melodic contour can
be realized 

These types of reconstruction problems arise in the analysis of
musical rhythm\ldots
}
...
information case.  In \secref{partial-information} we given an
algorithm for the sequential information case.  \Secref{conclusions}
...
\begin{algorithmic}[1]
\STATE{$r\gets\gcd(k,\ell_i)$}
\FOR{$c = 0,\ldots,k/r-1$}
  \STATE{$m \gets \infty$}
  \FOR{$j = 1,\ldots,r$}
    \STATE{$m \gets \min\{m, D_{i-1}((c-j\ell_i)\bmod k)+j\ell_i\}$}
  \ENDFOR
  \FOR{$j = 0,\ldots, r-1$}
    \STATE{$D_i((c+j\ell_i)\bmod k)\gets m$}
    \STATE{$m \gets\min\{m, D_{i-1}((c+j\ell_i)\bmod k) \} + \ell_i$}
  \ENDFOR
\ENDFOR
\end{algorithmic}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Sequential Information}\seclabel{partial-information}
