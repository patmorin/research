...
\usepackage{ltexpprt,amsopn,amsfonts,graphicx}
...
However, with a careful layering use of pointers or fractional
cascading \cite{ae76,ae196}, the query time can be reduced to $O(\log
n)$ without increasing the space requirement by more than a constant
factor.  Range trees can also answer more general \emph{semigroup
queries} in which each point of $S$ is assigned a weight from a
commutative semigroup and the goal is to report the weight of all
points in the query range \cite{ae133,ae292}.
...
\mu_D(T^*) 
 ...
 & \ge & \sum_{v\in T}
	\Pr(v) \cdot\max
            \left\{\begin{array}{l}\mu_{D_{\mid s(v)}}(T_w(u)), \\
		       \mu_{D_{\mid s(v)}}(T_y(u))
            \end{array}\right\} - 1 \\
 & \ge & \sum_{v\in T}
	\frac{1}{2}\Pr(v)\cdot
               \left(\begin{array}{l} \mu_{D_{\mid s(v)}}(T_x(u)) \\
                             {} +  \mu_{D_{\mid s(v)}}(T_y(u)) 
               \end{array}\right) - 1 \\
...
\mu_D(T^*) 
  ...
  & &      {}  -  (\log\alpha)\cdot  \sum_{i=1}^{\infty}
		\sum_{v\in G_{i,t_i}}\Pr(v)\cdot\depth(v) - O(1)\\
  & \ge & (\log\alpha)\cdot\sum_{v\in T}\Pr(v)\cdot \depth(v) \\
  & &      {}  -  (\log\alpha)\cdot  \sum_{i=1}^{\infty}i\cdot\Pr(G_{i,t_i}) - O(1) \\
  & \ge & (\log\alpha)\cdot\sum_{v\in T}\Pr(v)\cdot \depth(v) \\
   & &      {} - (\log\alpha)\cdot   \sum_{i=1}^{\ceil{\log n}}i\gamma^i/2^{i-1} - O(1) \\
  & \ge &  (\log\alpha)\cdot\sum_{v\in T}\Pr(v)\cdot \depth(v) - O(1) \\
  & = & \Omega(C_1(S,D)) \enspace ,
\end{eqnarray*}
where the last inequality follows from the fact that $\gamma/2 < 1$ so
that the second summation is bounded by a constant. 
\end{proof}
...
set.  However, when we decompose these queries into four 2-sided
...
%2-sided 2-dimensional orthogonal range searching for comparison-based data
%structures.  By using a table of size $O(n^2)$, one immediately
%obtains an $O(n^2)$ space data structure that answers queries using at
%most $2\log n + O(1)$ comparisons.  Is there an $O(n\log n)$ space
...
\etal\ \cite{cdilm08} yield an optimal data structure with $O(n)$
size.
\end{op}
