...
\begin{abstract}
  ...
  $t$-spanners that we dub robustness.  A graph is an $f(k)$-robust
  $t$-spanner if, for all $k$, the removal of any $k$ vertices leaves a
  set of $n-f(k)$ vertices such that there is a $t$-spanning path between
  every pair of vertices in this set.  We prove that $f(k)$-robust
...
spanners with $O(rn)$ edges exist \cite{cz04,lns02,l99}.
...
\begin{proof}
  ...
  it contains a path that visits all vertices in $V\setminus S^+$ in order.
