...
at least $2^{\alpha i}/e$ elements, otherwise $G_i$ is \emph{small}.
(Euler's constant, $e$, appears here because of a maximization argument
that appears later.)  Let $I^+$ denote the index set of the large
...
\overline H 
 ...
 & \le & \sum_{i\in I^+}\Pr(\cup G_{i})\log(1/\Pr(\cup G_{i})) 
         + \sum_{i\in I^-}2^{\alpha i}/2^{i-1}\log(2^{i-1}/2^{\alpha i}) 
\end{eqnarray*}
...
\overline H
   & \le & \sum_{i\in I^+}\Pr(\cup G_{i})\log(1/\Pr(\cup G_{i})) 
         + \sum_{i=1}^\infty 2^{\alpha i}/2^{i-1}\log(2^{(1-\alpha)i}) \\
 &  =  & \sum_{i\in I^+}\Pr(\cup G_{i})\log(1/\Pr(\cup G_{i})) 
         + (1-\alpha)\sum_{i=1}^\infty i2^{\alpha i}/2^{i-1} \\
 &  =  & \sum_{i\in I^+}\Pr(\cup G_{i})\log(1/\Pr(\cup G_{i})) 
         + 2(1-\alpha)\cdot\sum_{i=1}^{\infty} i/2^{(1-\alpha)i} \\
 &  =  & \sum_{i\in I^+}\Pr(\cup G_{i})\log(1/\Pr(\cup G_{i})) 
         + 2(1-\alpha)\cdot\left(\frac{(1/2)^{1-\alpha}}{(1-(1/2)^{1-\alpha})^2}\right) \\
 & \le & \sum_{i\in I^+}\Pr(\cup G_{i})\log(1/\Pr(\cup G_{i})) 
         + (1-\alpha)\cdot O(1/(1-\alpha)^2) \enspace ,
\end{eqnarray*}
