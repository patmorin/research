...
\noindent\textbf{Remark:} A slightly stronger result than
\lemref{weak-dd} can be obtained by proving that there always exist
$d+1$ halving hyperplanes that have a point in common (the continuity
argument used in the proof of \lemref{construction} works, almost
without modification.)  The bound for $\alpha$ is then improved to
$\alpha\ge \pi+2\phi_d(2d+2)$, where $\phi_d(k)$ is the smallest value
such that the unit sphere $S^{d-1}$ can be covered with a centrally
symmetric set of $k$ disks of angular radius $\phi_d(k)$.  However, a
simple volume argument shows that $\phi_d(2(d+1))$ approaches $\pi$ as
$d$ increases, thus this bound is not significantly better than that
of \lemref{weak-dd}.

Next, we specialize to dimension 3 and obtain some nearly-tight
results, as well as a somewhat surprising result about halving planes
in $\mathbb{R}^3$. 
...
sphere $S^2$ onto rotations of $\mathbb{R}^3$ as follows: \ldots Note
that $f$ is continuous (small changes in $x$ result in small changes
in the resulting rotation) and antipodal (i.e,.  $f(-x)(p)=-f(x)(p)$).  
