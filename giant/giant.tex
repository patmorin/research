...
is a (geometric) $t$-spanner of $V$ if $G$ is connected and, for every
pair $x,y\in V$,
\[
...
An \emph{$r$-fault-tolerant} $t$-spanner, $G=(V,E)$, has the property that
...
\begin{proof}
  ...
  We first note that, that the probability that any particular $x_i\in
  S$ is expensive is at most $1/4k$.  This is $x_i$ is expensive if
  and only if $(r-i)\equiv 0 \pmod {2^{\ceil{\log(4k)}}}$.
  The probability of selecting $r$ with this property is only $1/2^{\ceil{\log(4k)}}
...
2-spanner.  In contrast, \thmref{general-lower-bound-1d} shows that any
