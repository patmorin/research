...
the $n\choose 2$ line segments defined by pairs of points in $S$.
...
the current paper we consider specifically the case when $k=n-c$ and
show that perimeter-based and area-based outlier removal problems can
be solved in $\Oruntime$ time.  Thus, for any fixed $c$, both
...
running time of our algorithm is optimal.
}{}

The remainder of the paper is organized as follows:
\secref{preliminaries} defines the notation and some previous results
that are used in subsequent sections. \secref{algorithm} describes the
outlier removal algorithm. Finally \secref{conclusions} concludes and
...
in which they occur.  That is, the $j$th node, $N_0^j$ on the right
most path of $T$ has the label $|G_0^j|$. 
...
removed from $\ch(S)$ by a solution whose solution tree is $T$.  To
obtain the points in the solution the algorithm can be augmented using
...
\FOR{$g=1$ to $d$}
  ...
  \ENDFOR
\ENDFOR
\vc{\RETURN{$\textsc{CombineSolutions}(X,d,k-j+1,c_1,\ldots,c_d)$}}
{\RETURN{$\textsc{CombineSolutions}(X)$}}
\end{algorithmic}
\end{minipage}
\vc{}{
   \end{center}
...
bound $m_i \le (3c)^{i+1}$.  Finally, we note the points of $S_c$
(which are never deleted) appear in at most $3m_{c-1}$ subproblems.
Putting all this together we obtain:
}{
Due to space constraints we omit the analysis of
\textsc{FindOptimalOfType} that proves:}
...
\[
     D_{j,\ell} = \max\{h(i_1,\ldots,i_j):
      \mbox{$1\le i_1$, $i_j\le \ell$, and $i_{j'+1} \ge i_{j'}+c_{j'}+1$
...
\end{enumerate}
}{the radius of the smallest disk containing $S\setminus S'$, the
diameter of $S\setminus S'$ or the variance of $S\setminus S'$.}
