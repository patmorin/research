...
%  We deal with Type~3(b) pairs by showing
%  that there are only $o(n^2)$ such pairs $u$ and $w$ for which need
%  to apply the weak condition $\|uw\|_G/\|uw\|\le 2$, for some
%  $alpha\in o_n(1)$.  For the remaining pairs, $\|uw\|_G/\|uw\| = 1
%  + O(1/c)$.  Thus, the total contribution of Type~3(b) Pairs to the
%  average stretch factor is at most
%  \[ \binom{n}{2}^{-1}\left((1+O(1/c))\binom{n}{2} + 2\cdot o(n^2)\right) = 1+o(1) \enspace .  
%  \]
%  We say that a Type~3(b) pair, $(u,w)$, with $u\in\Delta_i$ and $w\in
%  E_i$ is \emph{good} if the distance from $u$ to $E_i$ is greater
%  than $r_i$.  If $(u,w)$ is good, then
%  \[
%     \frac{\|uw\|_G}{\|uw\|} \le \frac{r_i+6r_i/c}{r_i} = 1+O(1/c) \enspace .
%  \] 
%  If a Type~3(b) pair is not good, then we say that it is \emph{bad}.
%  Thus, our goal is to show that there are $o(n^2)$ bad pairs.  Assume,
%  for the sake of contradiction, that this is not true, and there are
%  at least $\epsilon n^2$ bad pairs.  Our strategy is to show that this
%  implies the existence of a point $w\in V$ that is in many bad pairs
%  $(u,w)$ and that the existence of such a $w$ contradicts Property~5
%  of $k$-partitions.
%
%  Let $V_i'\subseteq V_i$ denote
%  the set points $u\in V_i$ that are part of some bad Type~3(b) pair.
%  Then the total number of bad pairs is upper-bounded by
%  \[
%    \sum_{i=1}^{n'} |V_i'|\cdot|E_i\cap V| \ge \epsilon n^2 \enspace .
%  \] 
%  Each term in this sum is at most $kn$ and there are only $n'\le \alpha
%  n/k$ terms, for some constant $\alpha >0$.  A straightforward
%  calculation shows that there must be at least $\epsilon
%  n/2k=\Omega(\epsilon n/k)$ values of $i$ such that
%  \[  
%     |E_i\cap V| \ge \epsilon^2 n/(2\alpha) = \Omega(\epsilon^2 n) 
%  \]
%  and 
%  \[
%      |V_i'| \ge \epsilon^2 k/(2\alpha) = \Omega(\epsilon^2 k) \enspace .
%  \]
%  Let $i_1,\ldots,i_\ell$, $\ell\in\Omega(n/k)$, denote the values of
%  $i\in\{1,\ldots,n'\}$ that satisfy the preceding conditions.
%
%  Now, 
%  \[
%     \sum_{j=1}^\ell |E_{i_j}\cap V| \ge \sum_{j=1}^\ell
%        \Omega(\epsilon^2 n) \ge \Omega(\epsilon^3 n^2/k) \enspace .
%  \]
%  By the pigeonhole principle, there must be some point $w\in V$
%  such that $w\in E_{i_j}$ for $\Omega(\epsilon^3n/k)$ values of $j$.
%  Let $E_{i_{1}'},\ldots,E_{i_t'}$ be the subset of $E_1,\ldots,E_\ell$
%  consisting of only the disks that contain $w$.  Let $r$ denote the
%  radius of the largest disk in this set and let $B$ denote the ball of
%  radius $2r$ centered at $w$.\ldots
