...
\begin{thm}[Shannon's Theorem]
  ...
  $\mathcal{P}^{-1}(a)=\{q\in\R^d:\mathcal{P}=a\}$, for any $a\in A$. Then,
...
for $S\cap\Delta_i^*$. This recursive process stops when the depth of
a node exceeds some predefined depth $k\le \log_{r/3} m$.
...
\begin{thm}\thmlabel{point-partition-tree}
  ...
  properties, for every $i\in\{0,\ldots,k\}$:
  \begin{enumerate}
    \item $\Xi(V_i)$ is a partition of $\R^d$,  
...
\begin{proof}
  ...
  \ge 1/2^{i}$.  Note that, because the maximum depth of a node
  in $T_{\mathcal{P},D}$ is $\delta\log_{r/3} m$, Condition~2 of
  \thmref{prob-partition-tree} implies that the depth of a node in $G_i$
  is at most $i/\log(r/3) + O(1)$.
...
\begin{proof}
  ...
  \[
     (1-O(n^{-\tau\gamma})) O(H(L)) + O(n^{-\tau\gamma}) O(\log n) =
        O(H(L)+1)  \enspace .
  \]
...
subdivision \cite{k83,st86,egs86,m90,as98} we can apply odds-on trees.
