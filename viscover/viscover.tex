...
have $\Omega(n^4)$ connected components \cite{oXX} (see \figref{quartic}).
...
and at most $2m$ vertices to $\VG(S)$ as follows (see \figref{vg}.b):
...
\lemref{count} to some visibility counting problems considered by Fischer
...
\begin{thm}[\cite{mXX,aeXX}]\thmlabel{triangle-inclusion}
...
\lemref{cover} in the partition tree of \thmref{triangle-inclusion}.  This
...
\cite{hXX}, \thmref{containment} is near-optimal.  Hopcroft's Problem takes
...
The data structure of \lemref{containment} combined with random
...
the data structure of \lemref{containment} using $k=m_s$ that allows us
...
Unfortunately, the results of \thmref{counting-a} become weaker if we try
...
Next we use \lemref{count} to show how to keep the space requirement
...
described in \lemref{count}.  For any point $p\in\R^2$, the number of
