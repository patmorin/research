...
configuration except that their directions are reversed and therefore the
...
\item The function $f^d_\alpha$ for $d>2$ is still unknown.  

The same arguments used here (plus the Centerpoint Theorem) show that
$f^d_\alpha(n) = 1$ for $\alpha < \pi$ and $f^d_\alpha(n)\ge
\ceil{n/(d+1)}$ for $\alpha\ge \pi$.  

Furthermore, if we take any $d$ orthogonal halving hyperplanes of an
$n$ point set and let $p$ denote the common intersection point of
these hyperplanes, then it is not hard to see that $D_\alpha(p)\ge
\ceil{n/2}$ for $\alpha \ge 3\pi / 2$.  Note that this result is not
tight (in terms of $\alpha$) even for $d=2$.

We conjecture that, for any $d$, there exists $d+1$ concurrent halving
hyperplanes that are parallel to the sides of a regular $d$-simplex.  (This
should be easy to prove:  Maybe a good exercise in understanding the
Borsuk-Ulam Theorem.)  This would imply that, for all $d$,
$f^d_\alpha(n) \ge n/2$ for $\alpha\ge 4\pi/3$.

This leads to an even stronger conjecture, namely:
\[
...
\item In this paper we have only considered $\alpha$-wedges mainly
because, for $d=2$, these are more or less the only interesting
