...
contains no points other than $u$ and $v$ (see \figref{gabrieldef}).  The
...
\begin{figure}
\begin{center}{\includegraphics[scale=0.7]{gabrieldef}}\end{center}
...
\begin{figure}
\begin{center}{\includegraphics{pearlexamples}}\end{center}
...
\begin{figure}
\begin{center}\includegraphics{wedgedef}\end{center}
...
\begin{proof}
Refer to \figref{staircase}.  Let $r>0$ be real and $k$ be a positive
...
The result then follows immediately from \lemref{chernoff}.
\end{proof}
...
$\EXP[N] \le n\ell^2 = d\ln n$ and, by \lemref{chernoff}, 
\[
...
conditioned on $N \le 2d\ln n$, the expected number of neighbours of $X_1$
in $S$ is small;
\[
   \EXP[N'|N \le 2d\ln n]
        \le 4(\ln(2d\ln n) + 1) 
...
let $k=(c\ln n)/(\ln\ln n)$.  By the second part of
\lemref{t-shape-maxima},
\[
  \begin{aligned}
    \PROB\left\{N'> k|N\le 2d\ln n\right\}  
    & = 4\PROB\left\{N'> \frac{c\ln n}{4(v+1)(\ln\ln n)}\cdot(4(v+1)) |N\le 2d\ln n\right\} \\
    & \le4\left
    ...
    & \le4\left
         ...
         \right)^{\ln\ln n} \\
    & \le
         4\left(\frac{\exp\left(\frac{c\ln n}{4(v+1)}\right)}
          {\left(\frac{c\ln n}{4(v+1)}\right)^{\frac{c\ln n}{4(v+1)}}} \right) \\
...
any edge of length greater than $\ell=\sqrt{d\ln n/n}$ is at most
\[
 \begin{aligned}
   4n(1-\ell^2/4)^{n-2} 
    &  =  4n\left(1-\frac{d\ln n}{4n}\right)^{n-2} \\
    & \le 4ne^{-(n-1)d\ln n/4n} \\
    & \le 4n^{1-(n-1)d/4n} \\
    & = 4n^{1-(1-1/n)d/4} \\
 \end{aligned}
\]
...
\thmref{yao-lower-bound} can be extended to show that Yao graphs
