...
\figref{gen-indep}.a).  In this case, observe that each $\Delta(v_i)$
...
interior of $P$ (see \figref{gen-indep}.b). In this case, there is a
...
\begin{figure}
  \begin{center}
   \begin{tabular}{cc}
     \includegraphics{gen-indep-1} & \includegraphics{gen-indep-2} \\
     (a) & (b) 
   \end{tabular}
  \end{center}
  \caption{The two cases in the Proof of \lemref{gen-indep}.}
  \figlabel{gen-indep}
\end{figure}
...
is similar to that taken in \secref{convex}.  We attempt to find a
...
chains and at most one additional edge ($e_2$).  See \figref{funnel}.
...
\begin{figure}
  \begin{center}
    \begin{tabular}{cc}
      \includegraphics{funnel-a} &  \includegraphics{funnel-b} \\
      (a) & (b)
    \end{tabular}
  \end{center}
...
P_i\cup\cdots\cup P_j$ into 4 pieces (See \figref{half-funnel}.b):
\begin{enumerate}
